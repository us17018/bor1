...
button_7: KeyboardButton = KeyboardButton(text='Я хочу сменить колличество попыток')
...
async def process_stat_command(message: Message):
    await message.answer(f'Всего игр сыграно: {user["total_games"]}\n'
                         ...
                         reply_markup=keyboard)
@dp.message(Text(text='Я хочу сменить колличество попыток'))
async def process_attems_command(message: Message):
    await message.answer('Хорошо ниже укажите колличество желаемых попыток в формате $(и то число которое вам нужно) к примеру: $5')


@dp.message(Text(contains='$'))
async def process_attems_command(message: Message):
    global ATTEMPTS
    ATTEMPTS = message.text[1:]
    ATTEMPTS = int(ATTEMPTS)
    print(ATTEMPTS)
    await message.answer('Вы успешно сменили кол-во попыток!',
                         reply_markup=keyboard)
# Смена диапазона
@dp.message(Text(text='Я хочу сменить диапазон угадываемых чисел'))
async def process_dip_command(message: Message):
    await message.answer('Хорошо,ниже укажите через "-" от какого и до какого числа требуется диапазон',
                         reply_markup=keyboard)
...
async def process_start_command(message: Message):
    global first_digit
    global second_digit
    first_digit1 = message.text.split('-')
    first_digit = first_digit1[0]
    second_digit = first_digit1[1]
    first_digit = int(first_digit)
    second_digit = int(second_digit)
    await message.answer('Вы успешно сменили диапазон!',
                         reply_markup=keyboard)
...
async def process_other_text_answers(message: Message):
    if user['in_game']:
        await message.answer(f'Мы же сейчас с вами играем. '
                             f'Присылайте, пожалуйста, числа от {first_digit} до {second_digit}')
    else:
